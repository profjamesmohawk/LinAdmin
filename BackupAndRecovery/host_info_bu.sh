...
OUT_FILE=$(hostname)_report_bu.html
...
spit_title "COMP-10018 Host Info Report - Backup and Recovery"
...
spit_start "Part A: tar (2 points)"
tar tf /tmp/books.tar | grep WarAndPeace
tar tf /tmp/books.tgz | grep WarAndPeace
spit_end

spit_start "Part B: cpio (1 point)"
cpio -t < /tmp/books.cpio | grep WarAnd
spit_end

spit_start "Part B: compressed cpio (2 points)"
gunzip -c /tmp/books.cpio.gz | cpio -t | grep WarAnd
spit_end

spit_start "Part C: Incremental Archives (2 points)"
tar tf /tmp/opt_set/level_1.tar
spit_end

spit_start "Part D: Contents of /home/alice (1 point)"
ls -l /home/alice
spit_end

spit_start "Part D: Migration Strategy (2 points)"
cat /tmp/migrate.txt
spit_end

spit_start "Part E: Puzzle (1 point)"
cat /tmp/puzzle.txt
spit_end
